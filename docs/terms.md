...
You must satisfy the account, age, and authorization requirements of your selected model provider. If a provider permits use by a minor only with a parent or guardian, that permission is required. Some providers may not offer paid API services to minors. You are responsible for your provider account, API key, provider charges, and compliance with the provider's terms. FrameReply does not sell model access.
...
To the maximum extent allowed by law, the maintainers and contributors are not liable for provider charges, lost data, generated output, messages sent by a user, service interruption, or indirect damages arising from use of FrameReply.
