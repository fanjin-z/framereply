...
struct PrivacyAndDataView: View {
    ...
    private var localDataSection: some View {
        settingsPanel(title: "On This Device", symbol: "iphone") {
            Text(
                "Chats, personas, and generated replies are stored locally. Zeptly has no proxy server, analytics, advertising, or tracking."
            )
    ...
    private func deleteAllLocalData() {
        do {
            ...
            try ZeptlyDataStore.deleteAllUserData(in: modelContext)
            try ChatRepository(context: modelContext).seedIfNeeded()
            try PersonaRepository(context: modelContext).seedPersonasIfNeeded()
        } catch {
            deleteAllError =
                "Some local data could not be deleted. Restart Zeptly and try again before sharing the device."
        }
    }
}
