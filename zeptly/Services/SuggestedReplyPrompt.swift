...
nonisolated enum SuggestedReplyPrompt {
    ...
    Memory rules
    Return only durable, contact-specific facts useful in future replies: relationships, preferences, people, lasting facts, or meaningful events and commitments. Exclude greetings, transient details, the user's attributes, unsupported inferences, and duplicates. Every change must cite 1–3 supplied message IDs with direct evidence. Add an explicit new fact. Update an active memory only when newer evidence explicitly corrects or changes it; conflict alone is insufficient. Archive an active memory only when evidence makes it obsolete without replacement. When uncertain, return no change.
