...
nonisolated struct ProviderDataConsentDisclosure: Equatable, Sendable {
    ...
    var permissionMessage: String {
        "Zeptly will send the messages, images, names, and drafts you select directly to \(provider.displayName), a third-party AI provider, to analyze chats and create replies."
    }

    var summary: String {
        "Zeptly sends selected screenshots or message text, participant names, chat context, and drafts to \(destinationDescription) to analyze conversations and generate replies. Your provider may retain request data under its policy and may charge your provider account. Zeptly does not operate a proxy server."
    }
}
...
final class ProviderDataConsentStore {
    ...
    private func key(for platform: ProviderPlatform) -> String {
        "zeptly.providerDataConsent.\(platform.rawValue).v1"
    }
}
